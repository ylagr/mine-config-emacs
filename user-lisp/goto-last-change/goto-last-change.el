...
(defun my-get-buffer-last-change-pos (buffer)
  "获取指定 BUFFER 中最后一次修改的位置和时间戳."
  (with-current-buffer buffer
    (let ((undo-list buffer-undo-list))
      ;; 找到 undo 列表中第一个代表实际修改的项 (不是 nil 或 marker)
      (while (and undo-list (not (or (consp (car undo-list)) 
                                    (stringp (car undo-list)))))
        (setq undo-list (cdr undo-list)))
      (let ((last-change (car undo-list)))
        (cond
         ((consp last-change) (car last-change)) ;; (beg . end) 情况
         (t nil)))))) ;; 如果没找到则返回 nil

(defun my-goto-global-last-undo-pos ()
  "通过遍历所有 Buffer 的 undo 列表，跳转到最近的一个修改点."
  (interactive)
  (let ((best-buffer nil)
        (best-pos nil)
        (latest-time nil)
	)
    ;; 注意：原生 undo-list 不带时间戳，若需精确全局排序，
    ;; 通常依赖 buffer 的 last-change-time 或记录 hook。
    ;; 这里演示跳转到最近操作过的 buffer 的最后修改点：
    (let ((recent-buffers (buffer-list)))
      (dolist (buf recent-buffers)
        (when (and
	       (buffer-file-name buf) ; 只看关联文件的 buffer
                   (not (minibufferp buf)))
          (let ((pos (my-get-buffer-last-change-pos buf)))
            (when (and pos (not best-buffer)) ; 找到第一个有修改记录的活跃 buffer
              (setq best-buffer buf)
              (setq best-pos pos)))))
      
      (if (and best-buffer)
	  ;; (and best-buffer best-pos)
          (progn
            (switch-to-buffer best-buffer)
	    (goto-last-change)
            ;; (goto-char best-pos)
            (message "跳转至 %s 的最后修改点" (buffer-name)))
        (message "未发现有效的修改记录")))))
